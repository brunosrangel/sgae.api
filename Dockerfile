...
# Restaura as dependências NuGet da solução
RUN dotnet restore SgaeSolution.sln

# Copia o restante do código fonte e compila a aplicação
COPY . .
WORKDIR "/src/Sgae.API"
RUN dotnet build "Sgae.API.csproj" -c Release -o /app/build

# Estágio 3: Publicação dos artefatos compilados
FROM build AS publish
RUN dotnet publish "Sgae.API.csproj" -c Release -o /app/publish /p:UseAppHost=false
